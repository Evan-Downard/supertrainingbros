...
def new_time_limit_step(self, action):
...
def new_reset(self, **kwargs):
    ...
    if type(val) is tuple:
        ...
        return self.observation(val), {}
    
gym.ObservationWrapper.reset = new_reset
gym.wrappers.TimeLimit.step = new_time_limit_step
...
def run(training_mode, pretrained):
    ...
    for ep_num in tqdm(range(num_episodes)):
        ...
        while True:
            ...
            state_next, reward, terminal, truncated, info = env.step(int(action[0]))
            total_reward += reward
...
agent = run(training_mode=True, pretrained=False)
